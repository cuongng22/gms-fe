...
import {
	CategoryEnum,
	PlanCategoryEnum,
} from '../../../budget-procurement/budget-procurement.model';
...
@Component({
	...
	imports: [
		...
		MatMenuModule,
		MatExpansionModule,
		EstimatedCostGeneralComponent,
		InternationalEstimatedCostOvernightComponent,
		DomesticEstimatedCostFlightRateComponent,
		InternationalEstimatedCostHotelComponent,
		InternationalEstimatedCostCarRentalComponent,
		DomesticEstimatedCostHotelComponent,
		DomesticEstimatedCostCarRentalComponent,
...
export class EstimatedCostSummaryDetailComponent
	...
	implements OnInit, AfterViewChecked, AfterViewInit
{
	...
	@ViewChild('panelCalculationBasisState', { static: false })
	panelCalculationBasisState: MatExpansionPanel; // II
	@ViewChild('panelFlightOvernightState', { static: false })
	panelFlightOvernightState: MatExpansionPanel; // tỉ lệ ngủ đêm

	@ViewChild('panelEstimateCostState', { static: false })
	panelEstimateCostState: MatExpansionPanel; // III
	@ViewChild('panelEstimateCostHotelState', { static: false })
	panelEstimateCostHotelState: MatExpansionPanel; // 3.1
	@ViewChild('panelEstimateCostCarRentalState', { static: false })
	panelEstimateCostCarRentalState: MatExpansionPanel; // 3.2

	@ViewChild('estimatedCostGeneral', { static: false })
	estimatedCostGeneral: EstimatedCostGeneralComponent; //I
	@ViewChild('domesticFlightRate', { static: false })
	domesticFlightRate: DomesticEstimatedCostFlightRateComponent; //II
	@ViewChild('internationalEstimatedCostOvernight', { static: false })
	internationalEstimatedCostOvernight: InternationalEstimatedCostOvernightComponent; //II
	@ViewChild('internationalEstimatedCostHotel', { static: false })
	internationalEstimatedCostHotel: InternationalEstimatedCostHotelComponent; // III 3.1
	@ViewChild('internationalEstimatedCostCarRental', { static: false })
	internationalEstimatedCostCarRental: InternationalEstimatedCostCarRentalComponent; //3.2
	@ViewChild('domesticEstimatedCostHotel', { static: false })
	domesticEstimatedCostHotel: DomesticEstimatedCostHotelComponent; //3.1
	@ViewChild('domesticEstimatedCostCarRental', { static: false })
	domesticEstimatedCostCarRental: DomesticEstimatedCostCarRentalComponent; //3.2
	...
	async getDetailSummary() {
		try {
			...
			this.estimatedCostGeneral.unitPriceDoubleHotel =
				this.dataDetail?.unitPriceDoubleHotel;
			this.estimatedCostGeneral.unitPriceSingleHotel =
				this.dataDetail?.unitPriceSingleHotel;
	...
	summaryData() {
		if (
			this.dataDetail.planFlightRates ||
	...
	async confirmSummaryData() {
		try {
			...
			const requestBody = {
				...
				earlyCheckinFlag:
					!!this.estimatedCostGeneral.formGroupDetail.controls.earlyCheckinFlag
						.value,
				lateCheckoutFlag:
					!!this.estimatedCostGeneral.formGroupDetail.controls.lateCheckoutFlag
						.value,
				haveContract:
					!!this.estimatedCostGeneral.formGroupDetail.controls.haveContract
						.value,
				earlyCheckinContractFlag:
					!!this.estimatedCostGeneral.formGroupDetail.controls
						.earlyCheckinContractFlag.value,
				lateCheckoutContractFlag:
					!!this.estimatedCostGeneral.formGroupDetail.controls
						.lateCheckoutContractFlag.value,
			};
	...
	async processSave(): Promise<{ result: boolean; isUpdate?: boolean }> {
		try {
			...
			if (
				this.estimatedCostGeneral.formGroupDetail.invalid ||
				(this.category() === CategoryEnum.INTERNATIONAL &&
					this.internationalEstimatedCostOvernight.invalid())
			) {
			...
			if (this.category() === CategoryEnum.DOMESTIC) {
				planFlightRates = [
					...this.cleanData(this.domesticFlightRate.dataSource.data ?? []),
				];
				planBudgetHotels = [
					...this.cleanData(
						this.domesticEstimatedCostHotel?.getDataSource() ?? [],
					),
				];
				planBudgetCarentals = [
					...this.cleanData(
						this.domesticEstimatedCostCarRental?.dataSource.data ?? [],
					),
				];
				...
				planOverightRates = [
					...this.cleanData(
						this.internationalEstimatedCostOvernight.dataSource.data ?? [],
					),
				];
				planBudgetHotels = [
					...this.cleanData(
						this.internationalEstimatedCostHotel?.getDataSource() ?? [],
					),
				];
				planBudgetCarentals = [
					...this.cleanData(
						this.internationalEstimatedCostCarRental?.getDataSource() ?? [],
					),
				];
			}
	...
	cleanData(data: any[]): any[] {
		return data.map((item: any) => ({
			...item,
			id: !item.id || item.id < 0 ? null : item.id,
		}));
	}
	...
	async completed() {
		...
		if (resSave.result) {
			this.baseService
				.summaryUpdateStatus({
					id: this.id(),
					status: StatusSummaryEnum.COMPLETED,
				})
				.then(() => {
	...
	overnightValueChange(event: any): void {
		...
		this.internationalEstimatedCostHotel.setOvernightRates(
			event,
			event.actionType,
			this.internationalEstimatedCostOvernight.dataSource.data,
		);
	...
	setDataDetail(isSummary: boolean = false) {
		...
		this.estimatedCostGeneral.unitPriceSingleHotel =
			this.dataDetail?.unitPriceSingleHotel;
		this.estimatedCostGeneral.formGroupDetail.controls.crewTransportFeeFlag.setValue(
			this.dataDetail?.crewTransportFeeFlag,
		);
		...
		// }
		this.internationalFlightOvernightData = [
			...(this.dataDetail?.planOverightRates ?? []),
		];
