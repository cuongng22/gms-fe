	...
	implements OnInit, AfterViewChecked
{
	...
	setTotal(control: string) {
		...
		let totalValue = 0;
		if (
			this.type() === PlanCategoryEnum.BUDGET &&
			control === 'totalAmountVat'
		) {
			// const endDatePlanGroup = new Date(this.yearPlan(), 10, 1);
			totalValue = this.dataSource.data
				.map((t: any) => {
					if (
						truncateDate(new Date(t['periodStart'])) <=
						truncateDate(this.endDatePlanGroup)
					) {
		...
		totalValue = this.dataSource.data
			.map((t: any) => {
				if (
					truncateDate(new Date(t['periodStart'])) >=
					truncateDate(this.startDatePlanGroup)
				) {
					return round(Number(t[control]));
				}
				return 0;
	...
	clickOutside(data: any, control: string) {
		...
		if (
			[
				'priceSingleRoomEditing',
				'priceSingleRoomVatEditing',
				'priceDoubleRoomEditing',
				'priceDoubleRoomVatEditing',
				'singleRoomExtraEditing',
				'doubleRoomExtraEditing',
				'singleRoomEditing',
				'doubleRoomEditing',
				'singleRoomYearPerformEditing',
				'doubleRoomYearPerformEditing',
			].includes(control)
		) {
