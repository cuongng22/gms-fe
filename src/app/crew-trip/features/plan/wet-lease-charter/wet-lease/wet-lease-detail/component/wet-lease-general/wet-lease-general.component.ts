	...
	implements OnInit, AfterViewChecked
{
	...
	dataSourceHotel: MatTableDataSource<any, MatPaginator> =
		new MatTableDataSource<any, MatPaginator>();
	displayedColumnsHotel: string[] = [
		'supplierName',
		'unitPriceForSingleRoom',
		'unitPriceForTwinRoom',
		'action',
	];

	dataSourceCarRental: MatTableDataSource<any, MatPaginator> =
		new MatTableDataSource<any, MatPaginator>();
	...
	override async ngOnInit() {
		...
		await this.loadListFlightMarket({
	...
	checkDateInMonth() {
		...
		this.formGroupDetail.controls.endDate.updateValueAndValidity({
			emitEvent: false,
		});
	...
	async getExchangeRate() {
		if (
			!this.checkDateInMonth() &&
			this.formGroupDetail.controls.airportCode.value &&
			this.formGroupDetail.controls.startDate.value &&
			this.formGroupDetail.controls.endDate.value
		) {
			const response = await this.wetLeaseService.exchangeRate({
				startDate: this.dataTransformPipe.transform(
					this.formGroupDetail.controls.startDate.value,
					[this.Constant.DATE, this.Constant.LOCAL_DATE_FORMAT],
				),
				endDate: this.dataTransformPipe.transform(
					this.formGroupDetail.controls.endDate.value,
					[this.Constant.DATE, this.Constant.LOCAL_DATE_FORMAT],
				),
	...
	duplicateSupplierHotel() {
		if (this.dataSourceHotel.data) {
			return this.dataSourceHotel.data.some(
				(item, index) =>
					this.dataSourceHotel.data
						.map((mapItem) => mapItem.hotelCode)
						.indexOf(item.hotelCode) !== index,
			);
		}
		return false;
	}
	...
	checkRequiredHotelAndTransportation() {
		...
		this.isRequiredUnitPriceIncludingVatTransport =
			this.dataSourceCarRental.data.some((item: any) => !item.unitPrice);
	...
	duplicateCarRental() {
		if (this.dataSourceCarRental.data) {
			return this.dataSourceCarRental.data
				.map((item) => item.carRentalCode)
