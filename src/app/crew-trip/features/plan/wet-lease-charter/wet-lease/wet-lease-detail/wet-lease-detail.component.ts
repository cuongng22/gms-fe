...
@Component({
	...
	imports: [
		...
		MatMenuModule,
		MatExpansionModule,
		WetLeaseGeneralComponent,
		WetLeaseHotelComponent,
		WetLeaseCarRentalComponent,
	],
...
export class WetLeaseDetailComponent extends CommonComponent {
	...
	setPriceHotel() {
		...
		this.planHotel.forEach((element: any) => {
			Object.entries<any>(element.hotelItem).forEach(
				([hotelCode, hotelValue]) => {
					const _itemPrice = _priceHotelsList.find(
						(item) => item.hotelCode === hotelCode,
					);
	...
	setPriceCarRental() {
		...
		this.planTransports.forEach((element: any) => {
			const _itemPrice = _priceTransports.find(
				(item) => item.carRentalCode === element.transportCode,
			);
	...
	createData() {
		...
		const isRequiredHotelAndTrans =
			this.wetLeaseGeneral.checkRequiredHotelAndTransportation();
		const _dataGeneral = this.wetLeaseGeneral.formGroupDetail.getRawValue();
		if (
			this.wetLeaseGeneral.formGroupDetail.invalid ||
			...
			this.showError(
				$localize`:@@cannotCreateDataWithoutHotelOrTransportationData:Cannot create data without hotel or transportation data`,
			);
			return;
		} else if (
			(_dataGeneral.isHotel &&
				this.wetLeaseHotel.dataSource.data &&
				this.wetLeaseHotel.dataSource.data.length > 0) ||
			(_dataGeneral.isTransport &&
				this.wetLeaseCarRental.dataSource.data &&
				this.wetLeaseCarRental.dataSource.data.length > 0)
		) {
	...
	async confirmCreateData() {
		...
		let _startDate = moment(
			this.wetLeaseGeneral.formGroupDetail.controls.startDate.value,
		);
		let _endDate = moment(
		...
		this.dataGeneral = {
			...this.wetLeaseGeneral.formGroupDetail.getRawValue(),
		};
	...
	async saveAndProccess() {
		...
		if (res?.data && !this.id()) {
			this.router.navigate([
				'/plan/est-plan/wet-lease-charter/wet-lease-detail',
				res.data,
			]);
		} else if (this.id()) {
			this.getDetailById(this.id());
	...
	override async save(): Promise<any> {
		...
		if (
			...
			this.showError(
				$localize`:@@cannotSaveDataWithoutHotelOrTransportationData:Cannot save data without hotel or transportation data`,
			);
			return;
		}
		...
		_body.totalCountForeign = [
			..._planHotel.map((item) => item.totalCountForeign).flat(),
			..._planTransports.map((item) => item.totalAmountForex).flat(),
		].reduce((acc, value) => acc + value, 0);
		_body.totalSingleRoom = _planHotel
			.map((item) => item.totalQtySingleRoom)
			.reduce((acc, value) => acc + value, 0);
		_body.totalTwinRoom = _planHotel
			.map((item) => item.totalQtyTwinRoom)
			.reduce((acc, value) => acc + value, 0);
		_body.totalNumberOfTrip = _planTransports
			.map((item) => item.numberOfTrip)
			.reduce((acc, value) => acc + value, 0);
		_body.totalIncVat = [
			..._planHotel.map((item) => item.totalIncVAT).flat(),
			..._planTransports.map((item) => item.totalAmountIncVAT).flat(),
		].reduce((acc, value) => acc + value, 0);
		_body.totalExcVat = [
			..._planHotel.map((item) => item.totalExcVAT).flat(),
