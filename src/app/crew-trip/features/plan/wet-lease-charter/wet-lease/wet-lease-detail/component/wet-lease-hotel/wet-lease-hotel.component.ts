...
export class WetLeaseHotelComponent
	...
	totalPlannedBudgetRowDefCommon = [
		'plannedBudget',
		'ft2TotalQtySingleRoom',
		'ft2TotalQtyTwinRoom',
	...
	constructor() {
		...
		effect(() => {
			if (this.priceHotel() && (this.priceHotel()?.length ?? 0) > 0) {
				const _hotelHeaderRowDef1 = (this.priceHotel() ?? []).map(
					(item: any) => {
						return {
				...
				const _hotelHeaderRowDef2 = (this.priceHotel() ?? []).map(
					(item: any) => {
						return [
							`totalSingleRoom_${item.hotelCode}`,
							`totalTwinRoom_${item.hotelCode}`,
						];
					},
				);
	...
	override ngOnInit(): void {
		...
		this.roomPriceSubscription = this.baseService.roomPrice$.subscribe(
			(data) => {
				if (data) {
					this.totalPlannedBudget = {};
					this.dataSource.data.forEach((element) => {
						Object.entries<any>(element.hotelItem).forEach(
							([_hotelCode, _hotelValue]) => {
	...
	processColumnTable() {
		...
		let _hotelHeaderRowDef1: string[] = [
			...this.hotelHeaderRowDef1.map((item) => item.hotelCode),
		];
		let _rowDefCommon = [...this.rowDefCommon];
		let _totalRowDefCommon = [...this.totalRowDefCommon];
		let _totalPlannedBudgetRowDefCommon = [
			...this.totalPlannedBudgetRowDefCommon,
		];

		_headerRowDef1Common.splice(2, 0, ..._hotelHeaderRowDef1);
		this.headerRowDef1 = [..._headerRowDef1Common];

		this.headerRowDef2 = this.hotelHeaderRowDef2.concat(
			this.headerRowDef2Common,
		);
		...
		_totalPlannedBudgetRowDefCommon.splice(
			1,
			0,
			...this.hotelHeaderRowDef2
				.filter((item) => item.includes('totalSingleRoom'))
				.map((item) => 'ft2' + item),
		);
	...
	setTotalPlannedBudget(control: string, element: any) {
		...
		if (control.includes('_')) {
			...
			if (_formula) {
				...
				const result = this.calWithFormula(
					_formula,
					_hotelItem,
					this.dataGeneral(),
				);
	...
	getTotalRoom(element: any) {
		element.totalQtySingleRoom = Object.entries(element.hotelItem)
			.map((t: any[]) => Number(t[1].totalSingleRoom ?? 0))
			.reduce((acc, value) => acc + value, 0);
		element.totalQtyTwinRoom = Object.entries(element.hotelItem)
			.map((t: any[]) => Number(t[1].totalTwinRoom ?? 0))
			.reduce((acc, value) => acc + value, 0);
	}

	// tính tiền ngoại tệ
	calTotalCountForeign(element: any) {
		const _totalForex = Object.entries<any>(element.hotelItem)
