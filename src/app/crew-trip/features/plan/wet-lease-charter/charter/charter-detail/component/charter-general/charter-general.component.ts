	...
	implements AfterViewChecked, OnDestroy
{
	...
	override async ngOnInit() {
		...
		this.formGroupDetail.controls.priceHotel.valueChanges.subscribe(
			(_value) => {
	...
	duplicateCarRental() {
		if (this.dataSource.data) {
			return this.dataSource.data
				.map((item) => item.carType.toLowerCase())
	...
	checkRequiredTransportation() {
		this.isRequiredCarTypeTransportation = this.dataSource.data.some(
			(item: any) => !item.carType,
		);
		this.isRequiredPriceIncVatTransportation = this.dataSource.data.some(
			(item: any) => !item.priceIncVat,
		);

		return (
			this.isRequiredCarTypeTransportation ||
