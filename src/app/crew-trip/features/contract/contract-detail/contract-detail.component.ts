...
export class ContractDetailComponent extends CommonComponent implements OnInit {
  ...
  serviceTypeChange($event: any) {
    if ($event) {
      if ($event.value == 1) {
        this.formGroupDetail.patchValue({isHotel: true, isVehicle: false})
      } else if ($event.value == 2) {
        this.formGroupDetail.patchValue({isHotel: false, isVehicle: true})
      } else if ($event.value == 3) {
        this.formGroupDetail.patchValue({isHotel: true, isVehicle: true})
      }
      this.getPartnerInfo();
    }
  }
