...
export class InvoiceDocumentDetailComponent
  ...
  calRow(row: any, column?: any) {
    ...
    if (column == 'price') {
      //tinh don gian
      const amount = this.isInternational() ? +(row.getRawValue().amountFcBeforeVat ?? 0) : +(row.getRawValue().amountVndBeforeVat ?? 0);
      ...
      row.patchValue({
        amountVndBeforeVat: this.isInternational()
          ? this.roundUpNumber(
  ...
  async actionUpload() {
    try {
      await this.spinner.show();
      this.formGroupDetail.getRawValue().fileUpload.forEach((s: any) => {
