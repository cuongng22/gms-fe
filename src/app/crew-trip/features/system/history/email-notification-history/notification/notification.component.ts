...
@Component({
  ...
  imports: [
    ...
    MatFormField, MatInputModule, MatCheckboxModule
  ],
...
export class NotificationComponent extends CommonComponent {
  ...
  override displayedColumns: string[] = ['select', 'receiver', 'title', 'content', 'timeSend', 'status'];
  ...
  markAsRead() {
    if (this.selection.isEmpty()) {
      this.showError($localize`:@@:Cannot mask as read notifications if no notifications is selected`)
      return
    }
